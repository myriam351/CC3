...
```
### Commentaire : On peut Visualiser des taux d’erreurs estimés sous forme de graphique, via la fonction plotErrors(). Les points gris correspond aux erreurs selon dada2. On peut voir en premier lieu, le graphique A2C correspond à toutes les paires de bases A qui ont donné des C. En effet, chaque point gris correspond à un C du read. 

### Commentaire : Ici on a les taux d'erreurs estimées pour les amorces Foward.
...
```
### Commentaire : On peut Visualiser des taux d’erreurs estimés sous forme de graphique, via la fonction plotErrors(). Ici on a les taux d'erreurs estimées pour les amorces Reverse.
...
Cette fonction permet d’appliquer le modèle que Dada2 à créée pour tous les read forward du jeu de données, on aura donc plus besoin de faire d’OTU.
